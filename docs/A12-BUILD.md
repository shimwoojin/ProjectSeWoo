...
**메인 계정(`ggoggal627`)으로 SteamPipe 를 돌리지 않는다.** 지금은 그게 편하지만,
CI 에 자격 증명을 넣는 순간 **앱을 소유한 관리자 계정이 빌드 서버에 올라가 있는
상태**가 된다. 나중에 분리하려면 그때는 이미 여기저기 박혀 있다.
