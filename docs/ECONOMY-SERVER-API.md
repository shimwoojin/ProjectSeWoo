...
| 429 (레이트리밋, `Retry-After: 60`) — 세션 발급 IP당 분당 30, 나머지 스팀 ID당 분당 120 (server/README.md §3-6) | 지수 백오프 후 재시도. 지금 클라이언트(`EconomyClient`)는 실패 응답을 받으면 경고 로그를 남기고 서버 없음처럼 동작하다 다음 재동기화 때 다시 붙는다. 하베스트/구매는 사람의 조작 속도를 넘지 않으므로 정상 플레이에서 429를 볼 일이 없다 — 보인다면 매크로 의심 신호로 로그만 남긴다 |
