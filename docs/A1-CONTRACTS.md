...
| 날짜 | 무엇 | 사유 |
|---|---|---|
| 2026-09-23 (둘째) | `IInventoryService`에서 `Equip`/`EquippedIn` 제거 + 게임 레이어(`game/shop/Inventory.cs`, `GameRoot.cs`, `Tree.cs`) 리와이어링, 세이브 스키마 v4 | 처음 커밋한 `IInventoryService`가 "장착은 로컬"이라는 자기 문서 주석과 실제 시그니처가 어긋나 있었다 - 아무도 그 메서드를 쓰기 전이라 바로 고쳤다. 장착은 `SaveData.EquippedState`로 남기고, `game/`이 `IEconomyService`/`IInventoryService`를 실제로 소비하도록 바꿨다. `Tree`가 더 이상 성장 타이머를 소유하지 않고 서버(목) 값을 그리기만 한다. 세이브에서 `bananas`/`tree`/`upgrades`/`inventory.owned` 네 필드가 빠졌다(v4, 출시 전이라 마이그레이션 불필요). 상세: [ECONOMY-SERVER.md](ECONOMY-SERVER.md) §5-2·§6 |
| 2026-09-23 | `IEconomyService` + `IInventoryService` 추가 (10·11번째) | 기획서 §4-1("경제는 전부 로컬")을 뒤집는 결정 - 바나나로 산 커서 장식을 스팀 인벤토리로 옮기고 커뮤니티 마켓 거래를 노리기로 했다. 잔액이 실제 돈으로 거래될 아이템의 대가가 되는 순간 로컬 세이브를 신뢰할 수 없어진다. 상세 근거·아키텍처·백엔드 API 계약은 [ECONOMY-SERVER.md](ECONOMY-SERVER.md) / [ECONOMY-SERVER-API.md](ECONOMY-SERVER-API.md). 지금은 목만 있고 백엔드는 미착수 - `INetSession`이 A9~A11 전까지 목인 것과 같은 상태 |
| 2026-09-19 | `IInputSource` **의미 변경** — "타건 수" 에 마우스 버튼 클릭이 포함된다 (시그니처는 그대로) | 을 요청. 이 게임이 세려는 것은 "쏟은 시간" 인데 마우스만 쓰는 작업 중에는 원숭이가 멈춰 있었다. 휠은 제외(한 번 굴릴 때 이벤트가 여러 번 터진다), 초당 캡은 키와 **합산 10회** 유지(총 획득 속도가 변하면 §2-2 경제가 흔들린다), 감쇠 판정만 입력 종류별로 분리(섞으면 타자 중 도는 오토클리커가 빠져나간다). `TotalCount` 는 합계 하나 그대로라 `SaveData.TotalKeystrokes` 는 영향 없다. **개인정보 문구를 같이 고쳤다** — 기획서 §7-6 / STEAM-CONFIG.md §2-2. 마우스도 "어느 버튼인지·커서가 어디인지" 는 안 읽는다(마스크 `!= 0` 비교만). 레벨 곡선·마일스톤 수치는 A15 전 확정 작업으로 넘긴다. **을이 `platform/` 을 고쳤으므로 갑에게 통보 필요** |
