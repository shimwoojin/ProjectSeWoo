...
| 실물 아트 (16종) | `assets/cursor/` | ☑ **B9 완료 (2026-09-21)** — 아래 §2-1 |
...
호출부는 한 글자도 안 바뀌고** 실물 아트로 갈아탄다. 2026-09-21 에 실제로 그렇게
됐다 — 다만 한 줄이 걸렸다(§2-1). 그 전까지는 `icon.svg` 하나를
...
### 2-1. 실물이 들어오자 드러난 것 — 자리표시자 틴트 (2026-09-21)

`Equip` 이 **텍스처 종류와 무관하게** `TintFor(assetId)` 를 걸고 있었다. 해시 색은
"실물이 없을 때도 아이템을 구분해 보이게" 하려고 둔 것인데, 실물 아트에 곱하면
그림이 통째로 그 색이 된다 — B9 16종을 넣고 띄웠더니 **원숭이도 나무 단면도 전부
분홍으로 나왔다.**
