...
| 스팀 인벤토리 서비스 아이템 정의 등록 | `server/steam-inventory/itemdefs.json` | ☑ 15종(itemdefid 1~15) 등록·게시 완료. 아이콘은 GitHub raw URL(공개 저장소) 사용 — 더 안정적인 호스팅으로 나중에 옮기는 걸 고려할 것. 처음엔 `marketable: false`로 등록했다가 **2026-09-23 `marketable: true`로 재게시**(`09cb850`) — 마켓 신청 준비. 실제 거래 활성화는 밸브 승인이 따로 필요하다(§4) |
...
### 7-1. 실제 구현 상태 (2026-09-24, A14)

위 타협안 중 **"로컬 예측을 쌓는다"는 처음엔 구현돼 있지 않았다.** `EconomyClient`
의 슬롯은 서버 응답 순간의 스냅샷이라, 켤 때 익어 있던 바나나를 다 딴 뒤로 나무가
멈췄다(익은 슬롯이 없으면 하베스트 요청이 안 나가고, 요청이 안 나가면 새 스냅샷도
안 온다). 목은 `Tick` 으로 진행돼서 디버그에선 안 보였다. 실측(배포 서버, 20초):
고치기 전 슬롯 `37s` 고정 / 고친 후 `0→4→9→14→19s`.

| 항목 | 상태 |
|---|---|
| 응답 사이 로컬 성장 (`EconomyClient.AdvanceLocal`) | ☑ 서버 `recomputeSlots` 와 같은 규칙(다 자라면 멈춤) |
| 주기 재동기화 (`GameRoot.TickResync`) | ☑ 정상 5분, 서버 미연결·슬롯 0개면 30초 |
| HTTP 타임아웃 | ☑ 10초 (기본 100초면 오프라인 기동이 그만큼 멈췄다) |
| 기동 대기 상한 | ☑ 경제 동기화·인벤토리 조회 각 10초. 넘으면 있는 값으로 화면을 세운다 |
| 오프라인으로 켰을 때 | ☑ 슬롯 0개(빈 나무)로 시작하고 30초마다 재시도, 연결되면 채워진다. HUD 맨 아래에 "서버에 연결하는 중..." → "오프라인 - 연결되면 바나나가 열린다" (`StatusHud.SetNotice`) |
| 상점 "인터넷 연결 필요" 표시 | ☑ 미연결이면 안내 줄 + 구매 버튼 잠금(가격은 보인다). 상점을 열 때 미연결이면 바로 재동기화(세션 만료 직후 오판 방지). 구매 실패는 사유를 안내 줄에 띄운다 |
| 스팀보다 먼저 떴을 때 (자동 시작) | ☑ 보유 목록을 못 받은 동안(`IInventoryService.IsLoaded == false`)은 세이브의 장착을 믿는다 - 전에는 산 장식이 "안 가진 것" 으로 보여 **켤 때마다 장착이 지워지고 저장됐다.** 스팀이 붙으면 재동기화가 보유 목록을 다시 받아 진짜로 확인한다 |
| 스팀 밖에서 exe 직접 실행 | ☑ 릴리스에서만 `SteamAPI.RestartAppIfNecessary` → 스팀을 통해 다시 뜨고 종료 (`SteamService.RelaunchThroughSteamIfNeeded`). **익스포트 빌드로는 아직 안 돌려 봤다** |

> **계약 변경 (A1 §8-3 규칙): `IInventoryService.IsLoaded` 추가.** 목은 기본 true 라
> 게임 레이어 동작은 그대로다. 소유 여부로 무언가를 **지우는** 판단은 이 값이 true 일
> 때만 한다.
