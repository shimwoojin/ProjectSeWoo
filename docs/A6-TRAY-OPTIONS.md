...
> **2026-09-26 — "사운드" 체크박스와 `sound` 필드도 없앴다 (세이브 v8).** 소리를 지원하지 않기로 했다(B16 취소).
>
> **2026-09-26 — "타건 수 집계" 체크박스와 `keystrokeCounting` 필드를 없앴다 (세이브 v7).** 저장만 하고 읽는 코드가
> 없었다. 이유는 기획서 §7-4 의 메모.
